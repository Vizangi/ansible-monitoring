...
4.  **Мониторинг:** Prometheus на хосте собирает метрики с экспортеров в контейнерах, обращаясь к ним по их IP-адресам или именам на внутренней сети Podman. Grafana на хосте визуализирует эти метрики из Prometheus.

**Предварительные требования:**

На вашей Debian хост-машине должно быть установлено следующее:

1.  **Debian 12 (рекомендуется):** Podman лучше всего поддерживается в более новых версиях Debian.
...
│   │   ├── process_exporter/
│   │   │   └── tasks/
│   │   │       └── main.yml          # Задачи роли Process Exporter (внутри контейнера)
│   │   ├── nginx_exporter/
│   │   │   └── tasks/
│   │   │       └── main.yml          # Задачи роли Nginx Exporter (внутри контейнера)
│   │   ├── prometheus/
│   │   │   └── tasks/
...
То что выше уже готово! Ниже взять в работу. 
