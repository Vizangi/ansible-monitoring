...
- Нужно написать Terraform IaC темплейт для раворачивания x14 пустых контейнеров Docker с ОС Debian (уже написано и развернуто в [этом таске](https://github.com/lamjob1993/terraform-monitoring/blob/main/terraform/tasks/task_2.md)).
...
- Далее раскатываем Ansible-roles с экспортерами на 14 контейнеров Debian (в голове держим, что это VM).
- После успешной раскатки экспортеров на них, проверьте эндпоинты Prometheus по `remote-read` по всем контейнерам.
- Воспроизведите несколько дашбордов для проверки.
